...
- `MOGU_ED25519_PRIV=...` → the bot's secret (this README, §3). **Never commit it.**
...
| `MOGU_ED25519_PRIV` | yes | 32-byte seed hex from `cmd/keygen` — **secret** |
| `MOGU_GUILD_ID` | no | A guild ID → instant slash-command sync (else global sync, ~1h) |
| `MOGU_ROLE_ID` | no | Restrict `/getkey` to this role ID (unset = anyone) |
| `MOGU_DB` | no | SQLite path (default `mogu_keys.db`) |
| `MOGU_COOLDOWN` | no | Seconds between issues per user (default `10`) |
...
```ini
# /etc/systemd/system/mogu-bot.service
[Unit]
Description=MOGU license bot
After=network-online.target

[Service]
WorkingDirectory=/opt/mogu-bot
ExecStart=/opt/mogu-bot/.venv/bin/python bot.py
EnvironmentFile=/opt/mogu-bot/.env      # chmod 600; holds MOGU_ED25519_PRIV + DISCORD_TOKEN
Restart=always
RestartSec=5
User=mogubot
...
```
chmod 600 /opt/mogu-bot/.env
systemctl enable --now mogu-bot
journalctl -u mogu-bot -f
```

The bot only connects out (Discord gateway) — no inbound ports. Back up `mogu_keys.db`.
...
- Leaking `MOGU_ED25519_PRIV` lets anyone mint valid codes — protect it like a signing key.
- Code format (must match `lib/license/code.go`): `MOGU-` + base64url_nopad(
