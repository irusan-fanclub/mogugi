...
// mintcode mints a MOMETER code from MOMETER_ED25519_PRIV (mirrors the bot). Local testing only.
// Usage: mintcode <discordUserID> <displayName>
func main() {
	seed, err := hex.DecodeString(os.Getenv("MOMETER_ED25519_PRIV"))
	if err != nil || len(seed) != ed25519.SeedSize {
		fmt.Fprintln(os.Stderr, "set MOMETER_ED25519_PRIV to the 32-byte seed hex from cmd/keygen")
		os.Exit(1)
	}
	if len(os.Args) < 3 {
	...
	fmt.Println("MOMETER-" + base64.RawURLEncoding.EncodeToString(append(payload, sig...)))
}
